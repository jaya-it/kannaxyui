...
const listMessage = {
 ...
 let mode = global.opts['self'] ? 'Private' : 'Publik'
  ...
  let fkon = { key: { fromMe: false, participant: `${m.sender.split`@`[0]}@s.whatsapp.net`, ...(m.chat ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { contactMessage: { displayName: `${name}`, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;a,;;;\nFN:${name}\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`}}}
    ...
    // Membuka kunci enc. Jangan lupa support owner yah!
    let wibu = global.thumb
    let gambar = await(await fetch(wibu)).buffer()
    let buttonMessage= {
    'document':{'url':sgc},
    'mimetype':td,
    'fileName':global.wm,
    ...
    'body': 'www.instagram.com/jayasantika_26',
    'thumbnail':gambar,
    'sourceUrl':sgc}},
    'caption':text,
    'footer':botdate,
    'buttons':[
    {'buttonId':'.ping','buttonText':{'displayText':'Speed Bot'},'type':1},
